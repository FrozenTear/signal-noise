...
The project's own README is precise about scope: famfs is for fabric-attached memory generally and is "not CXL-specific in any way." CXL is the leading carrier today, but the filesystem is designed for the broader class. That distinction matters when you're reading the merge debate, because the objections aren't really about CXL — they're about whether the kernel should grow a new in-kernel filesystem to manage this memory class, or whether existing kernel infrastructure can be extended to do the same job.

Groves posted what is reported as the v10 patch series around the end of March 2026 — pin the exact date against the LKML `Date:` header before asserting it. He believed the code was close. LSFMM+BPF 2026 said not yet.
...
Until one of those is conclusively in or out, famfs v10 will wait for a merge window that keeps not arriving.
