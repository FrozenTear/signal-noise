...
///
/// Usage:
///   cargo run --bin db_admin --features server -- reject test-article "killed: thin sourcing"
...
async fn main() -> Result<()> {
    ...
    if args.len() < 3 || args.len() > 4 {
        bail!("Usage: db_admin <reject|delete> <slug> [reason]");
    }
    let cmd = &args[1];
    let slug = &args[2];
    ...
    match cmd.as_str() {
        "reject" => {
            let mut res = db
                .query(
                    "UPDATE article SET status = 'rejected', \
                     rejection_reason = $reason, updated_at = time::now() \
                     WHERE slug = $slug",
                )
            ...
            match reason.as_deref() {
                Some(r) => println!(
                    "Rejected {} article(s) with slug '{}' — reason: {}",
                    affected.len(),
                    slug,
                    r
                ),
                None => println!(
                    "Rejected {} article(s) with slug '{}' (no reason recorded)",
                    affected.len(),
                    slug
                ),
            }
